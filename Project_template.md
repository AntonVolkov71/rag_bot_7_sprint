...
- архитектурная диаграмма
  ...
  - [update.pluntuml](task6/answers/update.plantuml)
...
### Задание 7
- task7/golden set — фиксированный список вопросов с ожидаемым результатом:
  - answer — ответ должен быть найден,
  - unknown — система должна корректно ответить, что данных нет.
  - [golden_questions.tsv](task7/golden_questions.tsv)
  - скрипт запуска золотого набора
    - python /task7/evaluate_golden.py
    - [evaluate_golden.py](task7/evaluate_golden.py)

- из базы знаний были удалены:
  - characters/Топор Рыжий.md
  - locations/шалал-рай для милых.md
- При этом упоминания этих сущностей остались в других документах базы знаний.

- скрипт с использованием золотого набора прогнал дважды
  - до удаления 
    - [run_logs_before_removed.csv](task7/run_logs_before_removed.csv)
  - после удаление
    - [run_logs_after_remove.csv](task7/run_logs_after_remove.csv)
  
- результат 
  - базовые темы представленные в базе знаний обрабатываются корректно
  - до удаления все вопросы обрабатывались ожидаемо
  - после удаления выявлено, что retrieval может возращать связанные чанки из других тем
  - в логах зафиксированно, что found=1, но при этом видно что в наборе source нет файла, напрямую содержащий ответ 
      - пример запроса прямой и с упоминанием
        - ![Screenshot 2026-01-13 152345.jpg](task7/Screenshot%202026-01-13%20152345.jpg)
 
- вывод 
  - золотой набор выявил, что при удалении темы, она может содержаться в других
  - текущая логика опирается только на наличие чанков
  - для улучшения можно
    - ужесточить критерий "не знаю" и не отвечать на основе косвенных
    - учитывать соответствие сущности (entity) при поиске

### Docker и сервисы
- в директории /apps окончательная версия rag_bot
  - запуск сервисов [readme.md](apps/readme.md)