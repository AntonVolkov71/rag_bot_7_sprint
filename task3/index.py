...
def clean_chunks(chunks, min_chars: int = 200):
    out = []
    for c in chunks:
...
def build_entity_index(docs):
    # чтобы быстро искать exact-match entity в запросе
...
def main():
    ...
    chunks = clean_chunks(chunks, min_chars=200)
    ...
    # пересоздаём индекс (без ручного os.walk)
    ...
    test_queries = [
        ...
        "Кто такой Антон Подольский?",
        "Что такое счастье в голубе?",
    ]
