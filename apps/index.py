...
def switch_active_dir(new_active: Path) -> None:
    ...
    rel = new_active.relative_to(BASE_DIR)
    tmp.write_text(str(rel).replace("\\", "/"), encoding="utf-8")
...
def prepare_build_dir_unique(build_dir: Path, active_dir: Optional[Path]) -> None:
    build_dir.parent.mkdir(parents=True, exist_ok=True)

    if active_dir and active_dir.exists() and any(active_dir.iterdir()):
